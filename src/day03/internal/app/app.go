...
func RunServer() {
	...
	http.HandleFunc("/api/get_token", handlers.GetToken)
	http.HandleFunc("/api/recommend", handlers.Authenticate(func(w http.ResponseWriter, r *http.Request) { handlers.GetClosestPlacesHandler(w, r, store) }))
