...
var candyPrices = map[string]int64{
	"CE": 10,
	"AA": 15,
	"NT": 17,
	"DE": 21,
...
func buyCandyHandler(params BuyCandyParams) middleware.Responder {
	...
	if moneyReceived < 0 {
		return NewBuyCandyBadRequest().WithPayload(&BuyCandyBadRequestBody{
			Error: "Amount of money provided can't be negative",
		})
	}

	if *params.Order.CandyCount <= 0 {
		return NewBuyCandyBadRequest().WithPayload(&BuyCandyBadRequestBody{
			Error: "Candy count can't be negative or 0",
		})
	}

	if _, ok := candyPrices[*params.Order.CandyType]; !ok {
		return NewBuyCandyBadRequest().WithPayload(&BuyCandyBadRequestBody{
			Error: "Wrong candy type",
		})
	}

	if totalPrice <= moneyReceived {
		cCow := C.ask_cow(C.CString("Thank you!"))
		...
		return NewBuyCandyPaymentRequired().WithPayload(&BuyCandyPaymentRequiredBody{
			Error: fmt.Sprintf("You need %d more money", totalPrice-moneyReceived),
		})
	}
}